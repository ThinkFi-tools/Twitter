...
model_path = "https://drive.google.com/drive/folders/1RNIhB4wPWnleKULv_3a3-675r5rB9FhD?usp=sharing"
sentiment_task = pipeline("sentiment-analysis", model=model_path, tokenizer=model_path)
model_path = "https://drive.google.com/drive/folders/1i1zJs213r_C2qt5rGKAtBbiZW1T2cm1G?usp=sharing"
...
def get_followers(id, followers_path, limit=25000):
    rows = []
    while 1:
        try: 
            followers = tweepy.Cursor(api.get_follower_ids, screen_name=id,count=5000).items(limit)
            break
        except:
...
def get_following(id, following_path, limit=25000):
    # columns = ['username', 'userid', 'name', 'description', 'verified', 'created_at', 'followers', 'followings','location' , 'protected', 'profile_image_url']
    rows = []
    followings = tweepy.Cursor(api.get_friend_ids, screen_name=id,count=5000).items(limit)
